<?php
  ...
  if (!empty($_POST)) {
    ...
    $sql = "UPDATE esp32_table_temp_hum_update SET temperature = ?, humidity = ?, time = ?, date = ? WHERE id = ?";
    $q = $pdo->prepare($sql);
    $q->execute(array($temperature,$humidity,$tm,$dt,$id));
    Database::disconnect();
    //........................................ 
    ...
    while ($found_empty == false) {
      ...
      $sql = 'SELECT * FROM esp32_table_temp_hum_rec WHERE id="' . $id_key . '"';
      $q = $pdo->prepare($sql);
      $q->execute();
    ...
    // This table is also used to store and record the state of the LEDs, the state of the LEDs is controlled from the "home.php" page. 
    // This table is operated with the "INSERT" command, so this table will contain many rows.
		$sql = "INSERT INTO esp32_table_temp_hum_rec (id,board,temperature,humidity,time,date) values(?, ?, ?, ?, ?, ?)";
		$q = $pdo->prepare($sql);
		$q->execute(array($id_key,$board,$temperature,$humidity,$tm,$dt));
    //::::::::
